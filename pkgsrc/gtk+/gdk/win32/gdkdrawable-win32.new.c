			  ...
			  gint         height)
{
  ...
  if (gc_data->fill_style == GDK_OPAQUE_STIPPLED)
	...
	if (!WidenPath (hdc))
      ...
      if (ok && (hrgn = PathToRegion (hdc)) == NULL)
	WIN32_API_FAILED ("PathToRegion"), ok = FALSE;

      if (ok && (hbr = CreatePatternBrush (GDK_DRAWABLE_XID (gc_data->stipple))) == NULL)
	WIN32_API_FAILED ("CreatePatternBrush"), ok = FALSE;

      if (ok && !FillRgn (hdc, hrgn, hbr))
			...
			gint         npoints)
{
  ...
  if (gc_data->fill_style == GDK_OPAQUE_STIPPLED)
	...
	WIN32_API_FAILED ("CreatePatternBrush"), ok = FALSE;
      ...
      if (ok && !FillRgn (hdc, hrgn, hbr))
	WIN32_API_FAILED ("FillRgn"), ok = FALSE;
