			...
			GdkColormapPrivateWin32 *colormap_private)
{
  ...
  switch (data->fill_style)
    ...
    case GDK_OPAQUE_STIPPLED:
      /* Do nothing; CreatePatternBrush works OK, but if the stipple
       * isn't 8x8, selecting the resulting brush into the DC will
       * fail.  Yes, the Platform SDK documentation says nothing about
       * this (it only says that on Win9x it should be max 8x8), but
       * it works like this on my Win98. Maybe it's the display
       * drivers (I have a Riva TNT). Instead, the drawing functions
       * that can handle stipples do not call FillPath, but convert
       * the path into a region and call FillRgn.
       */
      break;
#endif
	...
	WIN32_API_FAILED ("CreateSolidBrush");
      else if (SelectObject (data->xgc, hbr) == NULL)
