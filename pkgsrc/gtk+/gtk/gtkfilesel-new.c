				 ...
				 const gchar      *filename)
{
  ...
  osfn = gdk_mbstoosfn (filename);

  last_slash = strrchr (osfn, G_DIR_SEPARATOR);
  ...
  if (!last_slash)
    ...
    {
      gint len = MIN (MAXPATHLEN - 1, last_slash - osfn + 1);

      strncpy (buf, osfn, len);
      buf[len] = 0;
  ...
  g_free (filesel->os_selection);
  filesel->os_selection = g_strdup (name);
  if (filesel->selection_entry)
    {
      name = gdk_osfntombs (name);
			     ...
			     const gchar      *pattern)
{
  ...
  osfn = gdk_mbstoosfn (pattern);
  g_free (filesel->os_selection);
  filesel->os_selection = g_strdup (osfn);
  gtk_file_selection_populate (filesel, osfn, TRUE);
  g_free (osfn);
}
...
gtk_file_selection_create_dir_confirmed (GtkWidget *widget, gpointer data)
{
  ...
  osfn_dirname = gdk_mbstoosfn (dirname);
  cmpl_state = (CompletionState*) fs->cmpl_state;
  path = cmpl_reference_position (cmpl_state);
  
  full_path = g_strconcat (path, G_DIR_SEPARATOR_S, osfn_dirname, NULL);
...
gtk_file_selection_delete_file_confirmed (GtkWidget *widget, gpointer data)
{
  ...
  osfn_full_path = gdk_mbstoosfn (full_path);
  if ( (unlink (osfn_full_path) < 0) ) 
    {
  ...
  if (event->keyval == GDK_Tab)
    {
      ...
      g_free (fs->os_selection);
      fs->os_selection = gdk_mbstoosfn (text);
      
      gtk_file_selection_populate (fs, fs->os_selection, TRUE);
			       ...
			       gpointer user_data)
{
  ...
  gchar *filename, *temp = NULL, *osfilename;
  ...
  temp = g_list_nth (fs->os_file_list, row)->data;
  osfilename = g_strdup (temp);
  ...
  if (filename)
	...
	switch (bevent->type)
	  ...
	  default:
	    ...
	    g_free (fs->os_selection);
	    fs->os_selection = g_strdup (osfilename);
	    break;
	  }
      else
	{
			       ...
			       gpointer user_data)
{
  ...
  filename = g_strdup (temp);
  temp = g_list_nth (fs->os_dir_list, row)->data;
  osfilename = g_strdup (temp);
  ...
  if (filename)
	...
	switch (bevent->type)
	  {
	  case GDK_2BUTTON_PRESS:
	    gtk_file_selection_populate (fs, osfilename, FALSE);
	    break;
...
static void
win32_gtk_add_drives_to_dir_list(GtkFileSelection *fs)
{
  ...
  while (*textPtr != '\0') {
	...
	fs->os_dir_list = g_list_append (fs->os_dir_list, g_strdup (text[0]));
      }
			     ...
			     gint              try_complete)
{
  ...
  g_list_free (fs->os_dir_list);
  fs->os_dir_list = NULL;
  gtk_clist_freeze (GTK_CLIST (fs->file_list));
  gtk_clist_clear (GTK_CLIST (fs->file_list));
  list = fs->os_file_list;
  while (list)
    {
      g_free (list->data);
      list = list->next;
    }
  g_list_free (fs->os_file_list);
  fs->os_file_list = NULL;

  /* Set the dir_list to include ./ and ../ */
  text[1] = NULL;
  text[0] = "." G_DIR_SEPARATOR_S;
  row = gtk_clist_append (GTK_CLIST (fs->dir_list), text);
  fs->os_dir_list = g_list_append (fs->os_dir_list,
				       g_strdup (text[0]));
  ...
  while (poss)
    {
      if (cmpl_is_a_completion (poss))
        {
          ...
          osfilename = cmpl_this_completion (poss);

	  filename = gdk_osfntombs (osfilename);
	  ...
	  text[0] = filename;
          ...
          else
	    {
	      ...
	      fs->os_file_list = g_list_append (fs->os_file_list,
						    osfilename);
  ...
  win32_gtk_add_drives_to_dir_list (fs);
#endif
  ...
  if (try_complete)
    {
      ...
      if (cmpl_updated_text (cmpl_state)[0])
		...
		{
		  gchar *upd_text = cmpl_updated_text (cmpl_state);
		  gchar *temp = gdk_osfntombs (upd_text);
		  
		  gtk_entry_set_text (GTK_ENTRY (fs->selection_entry),
				      temp);
		  g_free (fs->os_selection);
		  fs->os_selection = g_strdup (upd_text);
		  g_free (temp);
		}
            }
        }
	  ...
	  if (fs->selection_entry)
	    {
	      gchar *temp = gdk_osfntombs (rem_path);

	      gtk_entry_set_text (GTK_ENTRY (fs->selection_entry), temp);
	      g_free (fs->os_selection);
	      fs->os_selection = g_strdup (rem_path);
	      g_free (temp);
	    }
        }
    }
  ...
  if (!did_recurse)
    {
      if (fs->selection_entry)
	{
	  gchar *temp = gdk_osfntombs (cmpl_reference_position (cmpl_state));
