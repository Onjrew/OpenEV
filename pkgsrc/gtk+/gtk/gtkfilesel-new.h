...
/* The file selection widget needs to keep track of two versions of
 * file and directory names: In GTk+'s multi-byte representation, and
 * in the operating system's. On Unix, the issue is more or less
 * ignored but on Win32, this is a real problem. On Win32 GTk+ uses
 * Unicode and all strings are in UTF-8 representation. The operating
 * system however uses the default ANSI codepage for file system related
 * stuff.
 *
 * Below those fields which are in the operating system's representation
 * are prefixed with os_.
 */
