	      ...
	      gboolean     stat_subdirs)
{
  ...
  xdir = gdk_mbstoosfn (dir_name);
  path_buf_len = strlen (xdir);
  ...
  while ((dirent_ptr = readdir (directory)) != NULL)
    {
      entry_len = strlen (dirent_ptr->d_name);
      entry_count += 1;
  ...
  for (i = 0; i < entry_count; i += 1)
      ...
      if (!dirent_ptr)
	{
      ...
      xname = gdk_osfntombs (dirent_ptr->d_name);
      sent->entries[i].entry_name = xname;
			...
			CompletionState *cmpl_state)
{
  ...
  cmpl_state->the_completion.text =
    (gchar *) g_realloc (cmpl_state->the_completion.text, i);
