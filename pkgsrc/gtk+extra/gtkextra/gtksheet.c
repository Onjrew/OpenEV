...
{
  static GtkType sheet_range_type = 0;
  if (!sheet_range_type){
       static const GtkTypeInfo sheet_range_info =
       {
               "GtkSheetRange",
               0,
               0,
       ...
       sheet_range_type = gtk_type_unique(GTK_TYPE_BOXED, &sheet_range_info);
  }
...
gtk_sheet_construct (GtkSheet *sheet, guint rows, guint columns, const gchar *title)
{
  ...
  GTK_SHEET_SET_FLAGS(sheet, GTK_SHEET_JUSTIFY_ENTRY);
...
gtk_sheet_cell_draw_default (GtkSheet *sheet, gint row, gint col)
{
  ...
  if(sheet->show_grid && attributes.background.pixel == sheet->bg_color.pixel){
...
/* I'm emulating gtk_widget_size_request to avoid the redraw */
  ...
  if(GTK_IS_ITEM_ENTRY(sheet_entry)){
    gdk_gc_set_foreground(GTK_ITEM_ENTRY(sheet_entry)->fg_gc, 
                          &attributes.foreground);
    gdk_gc_set_foreground(GTK_ITEM_ENTRY(sheet_entry)->bg_gc,  
...
create_sheet_entry(GtkSheet *sheet)
{
 ...
 if(sheet->entry_type){
   ...
   if(!found_entry){
     ...
     entry=GTK_WIDGET(gtk_type_new(GTK_TYPE_ITEM_ENTRY));
...
gtk_sheet_button_draw (GtkSheet *sheet, gint row, gint column)
{
  ...
  if( (row == -1) && (column == -1) ) return; //This seems to cause a crash.
...
/* TODO: Should this really store the font pointer instead of the font value.
*/

void
gtk_sheet_range_set_font(GtkSheet *sheet, const GtkSheetRange *urange, GdkFont *font)
{
  gint i, j;
  gint font_height;
  GtkSheetCellAttr attributes;
  ...
  for (i=range.row0; i<=range.rowi; i++)
    for (j=range.col0; j<=range.coli; j++){
      gtk_sheet_get_attributes(sheet, i, j, &attributes);
      attributes.font = font; /* TODO: See comment above this function.*/
...
DeleteRow(GtkSheet *tbl, gint row, gint nrows)
{
  ...
  if(row <= tbl->maxallocrow){
    ...
    /* 
     * Ensure that there is always at least one allocated row, the above
     * logic can delete the last row even though maxallocrow is left at 1. 
     */
    if( tbl->data[tbl->maxallocrow] == NULL )
    {
      printf( "*** DeleteRow: Fixing up last row being NULL. ***\n" );
      tbl->data[tbl->maxallocrow] = (GtkSheetCell **) 
          g_malloc0( (tbl->maxalloccol+1)*sizeof(GtkSheetCell*)
                     + sizeof(double) );
    }
  }
  ...
  if(newcols>0){
      for(i=0; i <= tbl->maxallocrow; i++) {
          if( tbl->data[i] != NULL )
              ...
              tbl->data[i] = (GtkSheetCell **) \
                  g_malloc0((tbl->maxalloccol+1)*sizeof(GtkSheetCell *)+sizeof(double));
...
*/
      ...
      child->x = area.x;
      child->y = area.y;
      child->x += (area.width - child_requisition.width) * child->x_align; 
      child->y += (area.height - child_requisition.height) * child->y_align;
